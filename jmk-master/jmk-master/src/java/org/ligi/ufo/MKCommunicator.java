...
public class MKCommunicator
    ...
    private Vector<DUBwiseNotificationListenerInterface> notify_listeners = new Vector<DUBwiseNotificationListenerInterface>();
    ...
    private void connect() {
        ...
        try {
            ...
            log("Problem connecting" + "\n" + ex + "\nstats" + stats + "comm_adapter" + comm_adapter);
        }
    }
    ...
    public void clearWayPoints() {
        ...
        wp.setStatus(WayPoint.STATUS_INVALID);
        sendWayPoint(wp);
    }

    public void sendWayPoint(WayPoint wp) {
        send_command(NAVI_SLAVE_ADDR, 'w', wp.toByteArray());
    }
    ...
    public double dfollow_me_lat=0;
    public double dfollow_me_lon = 0;
    ...
    public void send_follow_me(byte time, int lon, int lat) {
        ...
        double currLat = Double.parseDouble(gps_position.Latitude_str());
        double currLong = Double.parseDouble(gps_position.Longitude_str());



        BlCalculations calc = new BlCalculations();
        bearing = calc.getBearing( dfollow_me_lat, dfollow_me_lon,currLat, currLong);
        ...
        MKHelper.int32ToByteArr((int) bearing, params, 13);

        //params[13] = d.byteValue();
       // params[14] = 0;  // heading
    ...
    public void send_follow_me(byte time, double lon, double lat) {
        long alt = 0;

        double currLat = Double.parseDouble(gps_position.Latitude_str());
        double currLong = Double.parseDouble(gps_position.Longitude_str());

        int lat1 = (int) (lat* 10000000);
        int lng1 = (int) (lon * 10000000);

        BlCalculations calc = new BlCalculations();
        bearing = calc.getBearing( lat, lon,currLat, currLong);

        byte[] params = new byte[29];

        MKHelper.int32ToByteArr(lat1, params, 0);
        MKHelper.int32ToByteArr(lng1, params, 4);
        MKHelper.int32ToByteArr(0, params, 8); // alt
    ...
    public void send_follow_me(byte time, double bearing ) {
        long alt = 0;

        double currLat = Double.parseDouble(gps_position.Latitude_str());
        double currLong = Double.parseDouble(gps_position.Longitude_str());

        int lat1 = (int) (currLat* 10000000);
        int lng1 = (int) (currLong * 10000000);
    ...
    public void setBLInfoInterval(int interval) {
        send_command(slave_addr, 'k', interval);
    }
    ...
    public void process_data(byte[] data, int len) {
        ...
        switch ((char) data[2]) {
            ...
            case 'W': // WayPoint accept
                last_confirmed_wp = (byte) (decoded_data[0]);
                break;
            ...
            case 'C':
                ...
                VesselData.centroid.setNick((byte) decoded_data[6]);
                VesselData.centroid.setRoll((byte) decoded_data[7]);
                VesselData.centroid.setYaw((byte) decoded_data[8]);
            ...
            case 'E':  // Error Str from Navi
                ...
                String new_error_str = "";
                for (int foo = 0; foo < 20; foo++)
                    if (decoded_data[foo] != 0)
                        new_error_str += (char) decoded_data[foo];

                error_str = new_error_str.replace("ERR: ", "");
                break;
            ...
            case 'X': // Waypoint ( requested by 'x' )
                ...
                WayPoint rcv_wp = new WayPoint(decoded_data, 2);

                log("Got WayPoint count:" + count + " pos:" + pos + " index:" + rcv_wp.getIndex());
            ...
            case 'K': // BL info
                BrushlessData bl_data = new BrushlessData(decoded_data, 0);
                log("got BL info " + bl_data.toString());
    ...
    public void run() {
        ...
        // initial "faked" timestamp to have proper timeouting
        last_data_timestamp = System.currentTimeMillis();
        ...
        while (thread_running) {
            ...
            /*else  if (slave_addr==FAKE_SLAVE_ADDR)
                ...
                recieving = true;
                int read_count;
                int avail = comm_adapter.available();

                if (avail < DATA_IN_BUFF_SIZE)
                ...
                // timeout - #karaka
                Log.i("reconnect_when_timeout" + reconnect_when_timeout);
                if (reconnect_when_timeout && (last_data_timestamp + NO_DATA_TIMEOUT < System.currentTimeMillis())) {
                ...
                log("Connected - reading data avail:" + avail + " read:" + read_count);
                //	pos=0;
                input = 0;
                ...
                log("Problem reading from MK -> closing conn exception:" + ex.toString());
                //log(ex.toString());
    ...
    private final static int NO_DATA_TIMEOUT = 3000;

    public boolean hasError() {
        if (last_data_timestamp + NO_DATA_TIMEOUT < System.currentTimeMillis())
            return true;
    ...
    public boolean isFlying() {
        if (is_navi())
            return gps_position.isFlying();
        // TODO read from flags
        return false; //stats.flying_time()>0;
    }
    ...
    public int getFlyingTime() {
        if (is_navi())
            return gps_position.FlyingTime;
        else
            return 0;
    }
}